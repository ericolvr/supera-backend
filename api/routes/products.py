...
@product_routes.get('/')
async def list_all(offset: int = 0, limit: int = 100, database: Session = Depends(get_database)):
...
async def update_by_name(name: str, new_data: ProductUpdate, token=Depends(logged_user), 
    database: Session = Depends(get_database)):
...
async def delete_by_name(name: str, token=Depends(logged_user),
    database: Session = Depends(get_database)):
